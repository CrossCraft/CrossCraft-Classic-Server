...
void Server::update(float dt, Core::Application *app) {
    ...
    for (auto &p : clients) {
        ...
        ptr->PlayerID = p.second->PlayerID;
        ptr->X = p.second->X;
        ptr->Y = p.second->Y;
        ptr->Z = p.second->Z;
        ptr->Yaw = p.second->Yaw;
        ptr->Pitch = p.second->Pitch;
    ...
    if (firstArg == "/kick" && op) {
        ...
        for (auto c : clients) {
            ...
            memcpy(ptr2->Message.contents, msgs.c_str(),
                   msgs.length() < STRING_LENGTH ? msgs.length()
                                                 : STRING_LENGTH);
        ...
        ptr1->PacketID = Outgoing::OutPacketTypes::eMessage;
        ptr1->PlayerID = 0;
        auto msg = std::string("&7[") + user + " -> " +
                   clients[player2]->username + "]: " + remaining;
        memset(ptr1->Message.contents, 0x20, STRING_LENGTH);
        memcpy(ptr1->Message.contents, msg.c_str(),
               msg.length() < STRING_LENGTH ? msg.length() : STRING_LENGTH);
...
auto Server::connection_listener(Server *server) -> void {
    while (!server->stopping) {
        ...
        SC_APP_INFO("New Connection from " + clientIP + " on port " +
